...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.vehicle-manufacture.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.vehicle-manufacture.*/) {print $3}')
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"2.2"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    echo "ERROR !!!! Pass the v2.x image tag"
  fi
}
